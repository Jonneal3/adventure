...
export default function HomePage() {
  ...
  // Note: Install check is now handled server-side in middleware.ts
  // This ensures immediate redirect to OAuth BEFORE any page loads (Shopify requirement)
  // If we reach this point, the store is already installed (middleware checked)
  useEffect(() => {
    const sp = new URLSearchParams(window.location.search);
    const shop = sp.get('shop');
    
    // If no shop param, we can't determine install status - just show UI
    if (!shop) { 
    ...
    // Store is installed (middleware already checked)
    // Just verify for UI state, but don't redirect
        ...
        <Layout.Section>
          <Card>
            <div style={{ padding: 16 }}>
              ...
              {checking ? (
                <Text as="p" variant="bodyMd">Redirecting to connect your store…</Text>
              ) : needsInstall ? (
                <div style={{ display: 'grid', gap: 16, alignItems: 'center', justifyItems: 'start' }}>
                  <Text as="p" variant="bodyMd">Connecting your Shopify store…</Text>
                  <Text as="p" variant="bodySm" tone="subdued">If you're not redirected automatically, click below:</Text>
