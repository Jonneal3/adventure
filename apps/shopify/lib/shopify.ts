// Use Web API URLSearchParams (available in both Node.js and Edge Runtime)
// No import needed - URLSearchParams is a global in both environments
 ...
 * @param returnUrl - URL to return to after merchant approves/declines
 * @param test - Whether this is a test charge
 */
export async function createRecurringCharge(
  ...
  returnUrl: string,
  test: boolean = false
): Promise<{ confirmation_url: string; id: number }> {
  const payload = {
  ...
  const resp = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/recurring_application_charges.json`, {
    method: 'POST',
    headers: {
      'X-Shopify-Access-Token': accessToken,
      'Content-Type': 'application/json',
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`createRecurringCharge failed: ${resp.status} ${text}`);
  }

  const json = await resp.json();
  return {
    confirmation_url: json.recurring_application_charge.confirmation_url,
    id: json.recurring_application_charge.id,
  };
}
...
export async function getRecurringCharge(
  ...
  accessToken: string,
  chargeId: number
): Promise<{ status: string; name: string; price: string }> {
  const resp = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/recurring_application_charges/${chargeId}.json`, {
    headers: {
      'X-Shopify-Access-Token': accessToken,
      'Content-Type': 'application/json',
    },
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`getRecurringCharge failed: ${resp.status} ${text}`);
  }
...
/**
 * List all recurring application charges
 */
export async function listRecurringCharges(
  shop: string,
  accessToken: string
): Promise<Array<{ id: number; status: string; name: string; price: string }>> {
  const resp = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/recurring_application_charges.json`, {
    headers: {
      'X-Shopify-Access-Token': accessToken,
      'Content-Type': 'application/json',
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`listRecurringCharges failed: ${resp.status} ${text}`);
  }

  const json = await resp.json();
  return json.recurring_application_charges || [];
}
...
export async function activateRecurringCharge(
  ...
  const resp = await fetch(`https://${shop}/admin/api/${SHOPIFY_API_VERSION}/recurring_application_charges/${chargeId}/activate.json`, {
    method: 'POST',
    headers: {
      'X-Shopify-Access-Token': accessToken,
      'Content-Type': 'application/json',
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`activateRecurringCharge failed: ${resp.status} ${text}`);
  }
}
...
export async function cancelRecurringCharge(
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`cancelRecurringCharge failed: ${resp.status} ${text}`);
  }
}
