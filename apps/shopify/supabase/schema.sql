...
-- oauth_states: temporary storage for OAuth state verification (fallback when cookies fail)
...
create index if not exists idx_oauth_states_shop on public.oauth_states using btree (shop) tablespace pg_default;
create index if not exists idx_oauth_states_expires_at on public.oauth_states using btree (expires_at) tablespace pg_default;

-- Clean up expired states periodically (can be done via cron job or manual cleanup)
