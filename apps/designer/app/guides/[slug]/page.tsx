...
export function generateStaticParams() {
  return TYPE_B_PAGES.filter((p) => p.kind === "guide").map((p) => ({ slug: p.slug }));
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const page = getTypeBPage("guide", params.slug);
    ...
    <div className="relative min-h-screen bg-background text-foreground overflow-hidden">
      <main className="mx-auto w-full max-w-[900px] px-4 md:px-8 pt-12 pb-16 md:pt-14 md:pb-20">
        <header className="max-w-[76ch]">
          <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Guide</p>
        ...
        {examples.length > 0 ? (
          <section className="mt-14">
            ...
            <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
              These are examples of the local (Type A) pages that can link to this guide.
            </p>
            <ul className="mt-4 space-y-2 text-[15px] md:text-[16px] text-muted-foreground">
              {examples.map((x) => (
                <li key={x.href}>
