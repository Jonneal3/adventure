...
export function AutoDemoOverlay({ onDismiss, config }: AutoDemoOverlayProps) {
  ...
  const steps: TooltipStep[] = useMemo(() => {
    const uploadMessage =
      (config.demo_upload_message && config.demo_upload_message.trim().length > 0
        ? config.demo_upload_message.trim()
        : 'Upload your reference images to guide the AI');
    const generationMessage =
      (config.demo_generation_message && config.demo_generation_message.trim().length > 0
        ? config.demo_generation_message.trim()
        : 'Your AI-generated images will appear here');

    return [
      { target: '[data-tour="upload-area"]', message: `📸 ${uploadMessage}`, position: 'bottom' },
      ...
      { target: '[data-tour="gallery-area"]', message: `✨ ${generationMessage}`, position: 'top' },
    ];
  }, [config.demo_upload_message, config.demo_generation_message]);
  ...
  return (
    <>
      {/* Elegant overlay */}
      <div 
        className="fixed inset-0 bg-black/30 backdrop-blur-[1px] z-[9999]"
        style={{ 
          pointerEvents: config.demo_click_to_dismiss ? 'auto' : 'none',
          cursor: config.demo_click_to_dismiss ? 'pointer' : 'default'
        }}
        onClick={(e) => {
          if (config.demo_click_to_dismiss) {
            e.stopPropagation();
            onDismiss();
          }
        }}
